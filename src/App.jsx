...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "40px 24px", background: "#fff" }}>
      ...
      <div style={{ marginBottom: 6, fontSize: 13, color: C.sub, textAlign: "center" }}>Welcome back — please sign in to your account</div>
      <div style={{ marginTop: 18 }}>
        <Field label="Username"><Input value={name} onChange={(e) => setName(e.target.value)} placeholder="Enter your name" /></Field>
        <Field label="Select role">
          <Select value={role} onChange={(e) => setRole(e.target.value)}>
            {ROLES.map((r) => <option key={r} value={r}>{r}</option>)}
          </Select>
        </Field>
        <Button full disabled={!name.trim()} onClick={() => onLogin({ name: name.trim(), role })} style={{ marginTop: 6 }}>Login</Button>
        <div style={{ fontSize: 11, color: C.faint, marginTop: 16, lineHeight: 1.6, textAlign: "center" }}>
          Prototype login — role controls what you see and can do. There's no password check yet;
          add real authentication before using this with live company data.
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ padding: "0 16px 16px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
        ...
        <QuickAction icon={UserCog} label="User Management" tone="warn" onClick={() => goto("more")} />
      </div>
    </div>
  );
}
...
function MorePage({ session, data, goto, onLogout }) {
  ...
  if (session.role === "Owner") items.push({ key: "settings", label: "Settings", icon: SettingsIcon });
  return (
    <div style={{ padding: "0 16px 16px" }}>
      <TopBar title="More" />
      <Card style={{ marginBottom: 14 }}>
...
export default function App() {
  ...
  function goto(key) { setPage(key === "more2" ? "more" : key); }
  ...
  if (!session) {
    return (
      <div style={outer}>
        <style>{`@import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap'); *{box-sizing:border-box;}`}</style>
        <div style={phoneFrame}><StatusBar /><LoginScreen onLogin={setSession} /></div>
      </div>
    );
  }
  ...
  const pages = {
    ...
    more: <MorePage session={session} data={data} goto={goto} onLogout={() => { setSession(null); setPage("dashboard"); }} />,
  };

  const showBottomNav = ["dashboard", "reports", "audit", "history", "more"].includes(page);
  ...
  return (
    <div style={outer}>
      ...
      <div style={phoneFrame}>
        ...
        {showBottomNav && <BottomNav items={nav} active={page === "settings" || page === "suppliers" || page === "customers" ? "more" : page} onChange={goto} />}
      </div>
    </div>
  );
}
